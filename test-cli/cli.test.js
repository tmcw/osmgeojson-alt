...
test("empty piped osm", function (t) {
  testCLI(t, 'echo "<osm/>" | ./osmtogeojson.js', function (geojson) {
...
test("piped osm", function (t) {
  ...
  t.plan(6);
  testCLI(t, 'echo "' + node + '" | ./osmtogeojson.js', function (geojson) {
    t.equal(geojson.features.length, 1);
    t.equal(geojson.features[0].geometry.type, "Point");
  });
  testCLI(t, 'echo "' + way + '" | ./osmtogeojson.js', function (geojson) {
    t.equal(geojson.features.length, 1);
    t.equal(geojson.features[0].geometry.type, "LineString");
  });
  testCLI(t, 'echo "' + relation + '" | ./osmtogeojson.js', function (geojson) {
    t.equal(geojson.features.length, 1);
...
test("piped json", function (t) {
  ...
  var way =
    '{"elements":[{"type":"way","id":1,"nodes":[2,3,4]},{"type":"node","id":2,"lat":0,"lon":1},{"type":"node","id":3,"lat":0,"lon":1.1},{"type":"node","id":4,"lat":0.1,"lon":1.2}]}';
  var relation =
...
test("piped osm (subformats)", function (t) {
  ...
  function compare_results(geojson) {
    if (first_result === undefined) first_result = JSON.stringify(geojson);
    else t.equal(JSON.stringify(geojson), first_result);
  }
  testCLI(t, "echo '" + xml + "' | ./osmtogeojson.js", compare_results);
  testCLI(t, "echo '" + xml + "' | ./osmtogeojson.js -f osm", compare_results);
  testCLI(t, "echo '" + xml + "' | ./osmtogeojson.js -f xml", compare_results);
  testCLI(
    t,
    "echo '" + xml + "' | ./osmtogeojson.js -f xmldom",
    compare_results,
  );
  testCLI(
    t,
    "echo '" + xml + "' | ./osmtogeojson.js -f fastxml",
    compare_results,
  );
  testCLI(
    t,
    "echo '" + xml + "' | ./osmtogeojson.js -f streamxml",
    compare_results,
  );
});
...
test("piped json (subformats)", function (t) {
  ...
  testCLI(t, "echo '" + json + "' | ./osmtogeojson.js", compare_results);
  testCLI(
    t,
    "echo '" + json + "' | ./osmtogeojson.js -f json",
    compare_results,
  );
  testCLI(
    t,
    "echo '" + json + "' | ./osmtogeojson.js -f nativejson",
    compare_results,
  );
  testCLI(
    t,
    "echo '" + json + "' | ./osmtogeojson.js -f streamjson",
    compare_results,
  );
});
...
test("empty osm file", function (t) {
  testCLI(t, "./osmtogeojson.js test-cli/data/empty.osm", function (geojson) {
    t.equal(geojson.features.length, 0);
    t.end();
  });
});
...
test("empty json file", function (t) {
  testCLI(t, "./osmtogeojson.js test-cli/data/empty.json", function (geojson) {
    t.equal(geojson.features.length, 0);
    t.end();
  });
});
...
test("osm file", function (t) {
  t.plan(6);
  testCLI(t, "./osmtogeojson.js test-cli/data/node.osm", function (geojson) {
    t.equal(geojson.features.length, 1);
    t.equal(geojson.features[0].geometry.type, "Point");
  });
  testCLI(t, "./osmtogeojson.js test-cli/data/way.osm", function (geojson) {
    t.equal(geojson.features.length, 1);
    t.equal(geojson.features[0].geometry.type, "LineString");
  });
  testCLI(
    t,
    "./osmtogeojson.js test-cli/data/relation.osm",
    function (geojson) {
      t.equal(geojson.features.length, 1);
      t.equal(geojson.features[0].geometry.type, "Polygon");
    },
...
test("json file", function (t) {
  t.plan(6);
  testCLI(t, "./osmtogeojson.js test-cli/data/node.json", function (geojson) {
    t.equal(geojson.features.length, 1);
    t.equal(geojson.features[0].geometry.type, "Point");
  });
  testCLI(t, "./osmtogeojson.js test-cli/data/way.json", function (geojson) {
    t.equal(geojson.features.length, 1);
    t.equal(geojson.features[0].geometry.type, "LineString");
  });
  testCLI(
    t,
    "./osmtogeojson.js test-cli/data/relation.json",
    function (geojson) {
      t.equal(geojson.features.length, 1);
      t.equal(geojson.features[0].geometry.type, "Polygon");
    },
...
test("osm file (subformats)", function (t) {
  ...
  testCLI(t, "./osmtogeojson.js ./test-cli/data/map.osm", compare_results);
  testCLI(
    t,
    "./osmtogeojson.js ./test-cli/data/map.osm -f osm",
    compare_results,
  );
  testCLI(
    t,
    "./osmtogeojson.js ./test-cli/data/map.osm -f xml",
    compare_results,
  );
  testCLI(
    t,
    "./osmtogeojson.js ./test-cli/data/map.osm -f xmldom",
    compare_results,
  );
  testCLI(
    t,
    "./osmtogeojson.js ./test-cli/data/map.osm -f fastxml",
    compare_results,
  );
  testCLI(
    t,
    "./osmtogeojson.js ./test-cli/data/map.osm -f streamxml",
    compare_results,
  );
});
...
test("json file (subformats)", function (t) {
  ...
  testCLI(t, "./osmtogeojson.js ./test-cli/data/map.json", compare_results);
  testCLI(
    t,
    "./osmtogeojson.js ./test-cli/data/map.json -f json",
    compare_results,
  );
  testCLI(
    t,
    "./osmtogeojson.js ./test-cli/data/map.json -f nativejson",
    compare_results,
  );
  testCLI(
    t,
    "./osmtogeojson.js ./test-cli/data/map.json -f streamjson",
    compare_results,
  );
});
...
test("overpass geometry types", function (t) {
  ...
  testCLI(t, "echo '" + xml + "' | ./osmtogeojson.js", function (geojson) {
  ...
  testCLI(t, "echo '" + json + "' | ./osmtogeojson.js", function (geojson) {
    t.equal(geojson.features.length, 2);
    t.equal(geojson.features[0].geometry.type, "Point");
    t.equal(geojson.features[1].geometry.type, "Point");
  });
  ...
  testCLI(t, 'echo "' + xml + '" | ./osmtogeojson.js -e', function (geojson) {
  ...
  json =
    ...
    "] }";
  testCLI(t, "echo '" + json + "' | ./osmtogeojson.js -e", function (geojson) {
    t.equal(geojson.features.length, 4);
    t.equal(geojson.features[0].properties.tainted, true);
    t.equal(geojson.features[1].properties.tainted, true);
    t.equal(geojson.features[2].properties.tainted, true);
...
test("parameters: -n", function (t) {
  var xml =
    "<osm><node id='1' lat='1.234' lon='4.321'><tag k='num' v='2' /></node></osm>";

  t.plan(2);
  testCLI(t, 'echo "' + xml + '" | ./osmtogeojson.js', function (geojson) {
    t.equal(geojson.features[0].properties.num, "2");
  });
  testCLI(t, 'echo "' + xml + '" | ./osmtogeojson.js -n', function (geojson) {
    t.equal(geojson.features[0].properties.num, 2);
  });
});
...
test("parameters: -e", function (t) {
  var xml =
    "<osm><node id='1' version='2' lat='1.234' lon='4.321'><tag k='k' v='v' /></node></osm>";
  ...
  testCLI(t, 'echo "' + xml + '" | ./osmtogeojson.js -en', function (geojson) {
    t.equal(geojson.features[0].properties.tags.k, "v");
    t.equal(geojson.features[0].properties.meta.version, 2);
  });
});
...
test("parameters: -m", function (t) {
  ...
  t.plan(2);
  testCLI_raw(t, 'echo "' + xml + '" | ./osmtogeojson.js', function (text) {
    t.equal(text.match(/\n/g).length, 21);
  });
  testCLI_raw(t, 'echo "' + xml + '" | ./osmtogeojson.js -m', function (text) {
    t.equal(text.match(/\n/g), null);
  });
});
...
test("parameters: --ndjson", function (t) {
  var xml =
    "<osm><node id='1' version='1' lat='0' lon='0'></node><node id='2' version='1' lat='1' lon='1'></node></osm>";
  ...
  testCLI_raw(
    t,
    'echo "' + xml + '" | ./osmtogeojson.js --ndjson',
    function (text) {
      t.equal(text.match(/\n/g).length, 2);
    },
  );
