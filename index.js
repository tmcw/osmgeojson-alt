...
osmtogeojson = function (data, options, featureCallback) {
  ...
  function _overpassJSON2geoJSON(json) {
    ...
    function centerGeometry(object) {
      var pseudoNode = structuredClone(object);
    ...
    function boundsGeometry(object) {
      var pseudoWay = structuredClone(object);
      pseudoWay.nodes = [];
    ...
    for (var i = 0; i < json.elements.length; i++) {
      switch (json.elements[i].type) {
        case "node":
          ...
          break;
        case "way":
          var way = structuredClone(json.elements[i]);
          way.nodes = structuredClone(way.nodes);
          ways.push(way);
          ...
          break;
        case "relation":
          var rel = structuredClone(json.elements[i]);
          rel.members = structuredClone(rel.members);
          rels.push(rel);
